...
from steam import ID, InvalidID, Type
import requests
...
class ProfileInfo:
    ...
    async def get(cls, data) -> Self:
        try:
            ...
            faceit_api = f'https://api.faceit.com/search/v2/players?query={steamid.id64}'
    ...
    def get_csgo_friend_code(steamid: ID) -> str | None:
        ...
        if steamid.type != Type.Individual or not steamid.is_valid():
            return

        h = b'CSGO' + struct.pack('>L', steamid.id)
